...
export const watermarkImg=(imgUrl,name)=>{
    const img = imgUrl?.indexOf('http')>-1 ? imgUrl : ''
    const shuiyin = img.indexOf(imageHost)>-1 ? `?x-oss-process=image/watermark,text_${base64url_encode(name)},t_30,fill_1,rotate_310,g_sw,size_100`:''
        return `${img}${shuiyin}`
}
